...
The following plot compares the Text-to-Token Ratio measured over wordform (TTR), lemma (LTR) and morph (MTR) per book of the New Testament. The immage clearly shows that shorter books generaly are resulting in higher ratio (even while TTR is already a ratio). There are various methods of normalization which are made available on Python in a conveniant package [lexicalrichness](https://lexicalrichness.readthedocs.io/en/latest/#)
